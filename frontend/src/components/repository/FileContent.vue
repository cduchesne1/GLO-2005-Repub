<template>
  <div>
    ...
    <div
      class="border border-solid border-gray-500 rounded-lg w-full mt-16"
      v-if="!isLoading"
    >
      <div
      ...
      <div class="text-white text-base px-4 py-4 whitespace-pre-wrap">
        {{ displayContent }}
      </div>
    </div>
  </div>
...
export default {
  ...
  async created() {
    ...
    if (this.fileName.includes(".md")) {
      this.displayContent = marked.parse(this.file);
    } else {
      this.displayContent = this.file;
    }
  },
};
</script>
